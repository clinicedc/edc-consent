...
class BaseSubjectConsentForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if hasattr(self._meta.model, 'ConsentAge'):
            ...
            rdelta = relativedelta(
                self.consent_datetime, cleaned_data.get('dob'))
    ...
    def accepted_consent_copy(self, cleaned_data):
        if (not cleaned_data.get('consent_copy', None)
                or cleaned_data.get('consent_copy', None) == NO):
            return False
        else:
            return True
    ...
    def consent_datetime(self):
        ...
        We validate consent age against the time of consent
        and not now().
        """
        try:
    ...
    def validate_age(self):
        ...
        if self.dob:
            ...
            if rdelta.years < settings.MINIMUM_AGE_OF_CONSENT:
                raise forms.ValidationError(
                    'Subject\'s age is {}. Subject is not eligible for '
    ...
    def validate_guardian(self, cleaned_data):
        ...
        if rdelta.years < settings.AGE_IS_ADULT:
            if "guardian_name" not in cleaned_data:
                raise forms.ValidationError(
                    'Subject is a minor. "guardian_name" is required '
                    'but missing from the form. Please add this field '
                    'to the form.')
            elif not cleaned_data.get("guardian_name", None):
                raise forms.ValidationError(
                    'Subject\'s age is {}. Subject is a minor. '
                    'Guardian\'s name is required here and with signature '
                    'on the paper document.'.format(
                        formatted_age(cleaned_data.get('dob'), date.today())))
            else:
                pass
        if (rdelta.years >= settings.AGE_IS_ADULT
                and "guardian_name" in cleaned_data.keys()):
            if not cleaned_data.get("guardian_name", None) == '':
                raise forms.ValidationError(
                    'Subject\'s age is {}. Subject is an adult. '
                    'Guardian\'s name is NOT required.'.format(
                        formatted_age(cleaned_data.get('dob'), date.today())))
    ...
    def validate_gender(self, cleaned_data):
        ...
        if cleaned_data.get('gender'):
            if cleaned_data.get('gender') not in settings.GENDER_OF_CONSENT:
                raise forms.ValidationError(
                    'Gender of edc_consent must be in {}. '
                    'You entered {}.'.format(
                        settings.GENDER_OF_CONSENT, cleaned_data.get(
                            'gender')))

    def validate_identity(self, cleaned_data):
        """Validate identity and confirm_identity match."""
        if cleaned_data.get('identity') and cleaned_data.get(
                'confirm_identity'):
            if cleaned_data.get('identity') != cleaned_data.get(
                    'confirm_identity'):
                raise forms.ValidationError(
                    'Identity mismatch. Identity number must match '
                    'the confirmation field.')

    def validate_audit_questions(self, cleaned_data):
        if (not cleaned_data.get('consent_reviewed', None)
                or cleaned_data.get('consent_reviewed', None) == NO):
            raise forms.ValidationError(
                'If edc_consent reviewed is No, patient cannot be enrolled')
        if (not cleaned_data.get('study_questions', None)
                or cleaned_data.get('study_questions', None) == NO):
            raise forms.ValidationError(
                'If unable to answer questions from client and/or None, '
                'patient cannot be enrolled')
        if 'assessment_score' in cleaned_data:
            if (not cleaned_data.get('assessment_score', None)
                    or cleaned_data.get('assessment_score', None) == NO):
                raise forms.ValidationError(
                    'Client assessment should at least be a passing score. '
                    'If No, patient cannot be enrolled')
        if not self.accepted_consent_copy(cleaned_data):
            raise forms.ValidationError(
                'If patient has not been given edc_consent copy and/or '
                'None, patient cannot be enrolled')
        if (cleaned_data.get('is_literate', None) == NO
                and not cleaned_data.get('witness_name', None)):
            raise forms.ValidationError(
                'You wrote subject is illiterate. Please provide the '
                'name of a witness here and with signature on the '
                'paper document.')
        if (cleaned_data.get('is_literate') == YES
                and cleaned_data.get('witness_name', None)):
            raise forms.ValidationError(
